...
public sealed class ControllerModelBuilder
{
    ...
    private readonly bool _suppressAsyncSuffixInActionNames;
    ...
    /// <param name="suppressAsyncSuffixInActionNames"></param>
    public ControllerModelBuilder(IModelMetadataProvider modelMetadataProvider, bool suppressAsyncSuffixInActionNames)
    {
        _modelMetadataProvider = modelMetadataProvider;
        _suppressAsyncSuffixInActionNames = suppressAsyncSuffixInActionNames;
    ...
    public ControllerModel Build()
    {
        ...
        {
            throw new ArgumentNullException("WithType need to be called");
        }

        var controllerModel = CreateControllerModel(_controllerType);
        controllerModel.Application = _applicationModel;

        if (_properties != null)
        {
            for (var i = 0; i < _properties.Count; i++)
            {
                var propertyModel = CreatePropertyModel(_properties[i]);
        ...
        if (_actions != null)
        {
            ...
            {
                var actionModel = CreateActionModel(_actions[i].Item1);

                if (actionModel != null)
                {
                    actionModel.ActionMethodInvoker = _actions[i].Item2;
                    actionModel.ActionMethodAwaitableInfo = _actions[i].Item3;
    ...
    /// <returns></returns>
    public ControllerModelBuilder WithControllerType(TypeInfo controllerType)
    {
    ...
    /// <returns></returns>
    public ControllerModelBuilder WithApplication(ApplicationModel applicationModel)
    {
    ...
    /// <returns></returns>
    public ControllerModelBuilder WithProperty(PropertyInfo propertyInfo)
    {
    ...
    /// <param name="propertyInfo"></param>
    /// <returns></returns>
    public ControllerModelBuilder WithProperties(PropertyInfo[] propertyInfo)
    {
        _properties ??= new List<PropertyInfo>();
        _properties.AddRange(propertyInfo);
    ...
    /// <returns></returns>
    public ControllerModelBuilder WithActions(MethodInfo[] actionMethods)
    {
        for (var i = 0; i < actionMethods.Length; i++)
        {
            _ = WithAction(actionMethods[i]);
        }
    ...
    private static ControllerModel CreateControllerModel(TypeInfo typeInfo)
    {
        if (typeInfo == null)
        {
    ...
    /// </returns>
    private ActionModel? CreateActionModel(MethodInfo methodInfo)
    {
        if (methodInfo == null)
        {
    ...
    private string CanonicalizeActionName(string actionName)
    {
        ...
        if (_suppressAsyncSuffixInActionNames &&
    ...
    /// </remarks>
    internal static bool IsAction(MethodInfo methodInfo)
    {
        if (methodInfo == null)
        {
            throw new ArgumentNullException(nameof(methodInfo));
